...
const ROYAL_ANIMALS = ['lion', 'whale', 'eagle', 'unicorn'];
let ROYAL_MAP = new Map(); // username -> animal
async function refreshRoyal() {
  try {
    const rows = await q.all(`SELECT username, animal FROM royal_users`);
...
app.get('/api/royal-info', requireUser, async (req, res) => {
  const settings = await getSettings();
  const price = Math.max(0, parseInt(settings.royal_entry_cost) || 50);
  const me = await q.get(`SELECT * FROM users WHERE id=?`, req.authUid);
...
app.post('/api/admin/gifts', requireSuperAdmin, async (req, res) => {
  ...
  if (id) await q.run(`UPDATE gifts SET name=?, img=?, audio=?, price=?, payout=?, cat=?, usd_value=? WHERE id=?`, n, im, au, pr, py, ct, Math.max(0, parseFloat(req.body.usd_value) || 0), +id);
  else await q.run(`INSERT INTO gifts (name,img,audio,price,payout,cat,usd_value) VALUES (?,?,?,?,?,?,?)`, n, im, au, pr, py, ct, Math.max(0, parseFloat(req.body.usd_value) || 0));
  io.emit('sync');
  res.json({ ok: true });
});
...
app.post('/api/admin/service-requests/:id/approve', requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    if (request.request_type === 'verify') {
      ...
      await q.run(`INSERT INTO royal_users (username, animal) VALUES (?,?)
        ON CONFLICT(username) DO UPDATE SET animal=excluded.animal`, target.username, royalAnimal);
      await refreshRoyal();
      await refreshUserEverywhere(target.id);
  ...
  const actionText = request.request_type === 'verify'
    ...
    : (request.request_type === 'royal'
      ? `👑 تمت الموافقة على الدخول الملكي لحسابك وخصم ${gold} ذهب — سيظهر التوهج الملكي عند دخولك الغرف`
...
app.get('/api/public-settings', async (req, res) => {
  ...
  const sanitized = {
    ...
    video_call_cost: Math.max(0, parseInt(s.video_call_cost) || 5),
