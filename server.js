...
app.delete('/api/wall/:id', requireUser, async (req, res) => {
  ...
  if (!post) return res.status(404).json({ error: 'المنشور غير موجود' });
  const me = await q.get(`SELECT rank FROM users WHERE id=?`, req.authUid);
  if (+post.user_id !== +req.authUid && (!me || !['admin', 'superadmin'].includes(me.rank)))
...
// بث الفيديو مقصور على الإدارة وأعضاء VIP/بريميوم/مميز (تماشياً مع ميزات العضويات المعلنة)
function canStartVideoBroadcast(user) {
  if (!user) return false;
  if (['roomadmin', 'admin', 'superadmin'].includes(user.rank)) return true;
  return ['vip', 'premium', 'mmez'].includes(user.membership);
}
// بث الصوت متاح لأي عضو مسجل (غير زائر) أو للإدارة
function canStartAudioBroadcast(user) {
  if (!user) return false;
  if (['roomadmin', 'admin', 'superadmin'].includes(user.rank)) return true;
...
io.on('connection', async (socket) => {
  ...
  socket.on('bcast:start', async (roomId, cb) => {
    ...
    const allowed = mode === 'video' ? canStartVideoBroadcast(me) : canStartAudioBroadcast(me);
    if (!allowed) return ack({
      ok: false,
      text: mode === 'video' ? '🔒 بث الفيديو المباشر متاح لأعضاء VIP/بريميوم/مميز والإدارة فقط' : 'يجب تسجيل حساب لبدء بث صوتي'
    });
  ...
  socket.on('bcast:speak_request', async (roomId, cb) => {
    ...
    me = await q.get(`SELECT * FROM users WHERE id=?`, uid);
    if (!canStartAudioBroadcast(me)) return ack({ ok: false, text: 'يجب تسجيل حساب لطلب التحدث' });
