...
// =====================================================
//  مفسّر حزم WebSocket المعمّى (Cloaked Socket.IO Parser)
// =====================================================
// يغلّف مفسّر Socket.IO في الاتجاهين بحيث تخرج كل حزمة عبر السلك مشفّرة تماماً:
//   {"_nv": "fYhHbz77AwygyR6CKlgYtH3DkiFl2lv_jD5cYmyMeLbJWogR"}
// فلا يظهر أي اسم حدث (join, msg, roomUsers, keepalive...) ولا أي نص رسالة
// أو معرف مستخدم أو غرفة في أدوات المطور أو أدوات الفحص أو أي التقاط للشبكة.
// أي حزمة تصل من اتصال خارجي غير معمّاة أو بمفتاح غير مطابق تُسقط فوراً
// ويفصل الخادم الاتصال مباشرة.

function createBuiltinParser() {
  function SimpleEmitter() {
    this._events = {};
  }
  SimpleEmitter.prototype.on = function (ev, fn) {
    (this._events[ev] = this._events[ev] || []).push(fn);
    return this;
  };
  SimpleEmitter.prototype.emit = function (ev) {
    var args = Array.prototype.slice.call(arguments, 1);
    var list = (this._events[ev] || []).slice();
    for (var i = 0; i < list.length; i++) list[i].apply(this, args);
    return true;
  };
  SimpleEmitter.prototype.emitReserved = SimpleEmitter.prototype.emit;
  SimpleEmitter.prototype.off = function (ev, fn) {
    if (!ev) { this._events = {}; return this; }
    if (!fn) { delete this._events[ev]; return this; }
  ...
  SimpleEmitter.prototype.destroy = function () {
    this._events = {};
  };

  function BuiltinEncoder() {}
  BuiltinEncoder.prototype.encode = function (obj) {
    var str = '' + obj.type;
    if (obj.type === 5 || obj.type === 6) {
      str += (obj.attachments || 0) + '-';
    }
    if (obj.nsp && obj.nsp !== '/') {
      str += obj.nsp + ',';
    }
  ...
  function BuiltinDecoder() {
    SimpleEmitter.call(this);
  }
  BuiltinDecoder.prototype = Object.create(SimpleEmitter.prototype);
  BuiltinDecoder.prototype.constructor = BuiltinDecoder;
  BuiltinDecoder.prototype.add = function (str) {
    if (typeof str !== 'string') return;
    var i = 0;
    var p = { type: Number(str.charAt(0)) };
    if (p.type === 5 || p.type === 6) {
      var start = i + 1;
      while (str.charAt(++i) !== '-' && i !== str.length) {}
      p.attachments = Number(str.substring(start, i));
    }
    if (str.charAt(i + 1) === '/') {
      var nspStart = i + 1;
      while (++i) {
      ...
      p.nsp = str.substring(nspStart, i);
    } else {
      p.nsp = '/';
    }
    ...
    if (next !== '' && Number(next) == next) {
      ...
      while (++i) {
        var ch = str.charAt(i);
        if (ch == null || Number(ch) != ch) { --i; break; }
        if (i === str.length) break;
      }
      p.id = Number(str.substring(idStart, i + 1));
    }
    if (str.charAt(++i)) {
      try { p.data = JSON.parse(str.substr(i)); } catch (e) { p.data = null; }
    }
    this.emit('decoded', p);
  };
  ...
  return {
    protocol: 5,
    PacketType: { CONNECT: 0, DISCONNECT: 1, EVENT: 2, ACK: 3, CONNECT_ERROR: 4, BINARY_EVENT: 5, BINARY_ACK: 6 },
    Encoder: BuiltinEncoder,
    Decoder: BuiltinDecoder
  };
}

function resolveDefaultParser(defaultParserOpt) {
  if (defaultParserOpt && defaultParserOpt.Encoder && defaultParserOpt.Decoder) {
    return defaultParserOpt;
  }
  if (typeof require !== 'undefined') {
    try {
      var reqParser = require('socket.io-parser');
      if (reqParser && reqParser.Encoder && reqParser.Decoder) return reqParser;
    } catch (e) {}
  }
  if (typeof window !== 'undefined' && window.io && window.io.Manager) {
    try {
      if (!window.__NUJUM_DEFAULT_PARSER__) {
        var dummy = new window.io.Manager({ autoConnect: false });
        window.__NUJUM_DEFAULT_PARSER__ = {
          protocol: 5,
          PacketType: { CONNECT: 0, DISCONNECT: 1, EVENT: 2, ACK: 3, CONNECT_ERROR: 4, BINARY_EVENT: 5, BINARY_ACK: 6 },
          Encoder: dummy.encoder.constructor,
          Decoder: dummy.decoder.constructor
        };
      }
      return window.__NUJUM_DEFAULT_PARSER__;
    } catch (e) {}
  }
  return createBuiltinParser();
}

function createSocketParser(keyHex, defaultParserOpt) {
  var def = resolveDefaultParser(defaultParserOpt);
  var DefaultEncoder = def.Encoder;
  var DefaultDecoder = def.Decoder;
  var isServer = typeof window === 'undefined';

  class CloakEncoder extends DefaultEncoder {
    encode(packet) {
      var encoded = super.encode(packet);
      for (var i = 0; i < encoded.length; i++) {
        if (typeof encoded[i] === 'string') {
          encoded[i] = JSON.stringify({ [BODY_ENVELOPE_KEY]: encodeCloakedValue(keyHex, encoded[i]) });
        }
      }
      return encoded;
    }
  ...
  class CloakDecoder extends DefaultDecoder {
    add(chunk) {
      if (typeof chunk === 'string') {
        try {
          var obj = JSON.parse(chunk);
          if (obj && typeof obj === 'object' && obj[BODY_ENVELOPE_KEY]) {
            var raw = decodeCloakedValue(keyHex, obj[BODY_ENVELOPE_KEY]);
            if (raw) {
              return super.add(raw);
            }
          }
        } catch (e) {}
        // اتصال خارجي أو حزمة غير معمّاة أو تالفة ← الخادم يقطع الاتصال فوراً
        if (isServer) {
          throw new Error('Uncloaked or invalid socket packet rejected');
        }
        return;
      }
