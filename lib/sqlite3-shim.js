...
function toCallback(err, val, cb) {
  ...
  // this=val يحاكي sqlite3 الأصلي حيث يحمل this في db.run آخر lastID/changes —
  // بدونه ينكسر كل كود يعتمد على نتيجة q.run (دخول الزوار، التسجيل، الهدايا...).
  process.nextTick(() => cb.call(val, err, val));
}

function bindParams(stmt, params) {
  // ملاحظة: لا نستخدم stmt.bind() لأنه يُستدعى مرة واحدة فقط لكل عبارة في
  // better-sqlite3، فينكسر أي كود يعيد استخدام العبارة (مثل زرع الإعدادات).
  // بدلاً منه نمرر البارامترات مباشرة إلى run/get/all التي تدعم إعادة الاستخدام.
...
class Statement {
  ...
  run(...args) {
    ...
    try {
      const info = raw.run(...bindParams(raw, args));
  ...
  get(...args) {
    ...
    try {
      const row = raw.get(...bindParams(raw, args));
  ...
  all(...args) {
    ...
    try {
      const rows = raw.all(...bindParams(raw, args));
