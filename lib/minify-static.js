...
// تصغير نصّ JS خام (مثل lib/cloak.js المخدوم خارج public/js). يكتفي بإزالة
// التعليقات والفراغات دون ضغط/إعادة تسمية — أمان مطلق للكود الحسّاس. يعيد
// النص الأصلي عند أي خطأ.
async function minifyJsText(text) {
  loadMinifiers();
  if (_terserMinify) {
    try {
      const out = await _terserMinify(text, {
...
// يعيد النص المصغّر لملف عام (مسار نسبي من public/)، أو null عند عدم الوجود.
async function minifiedText(relPath) {
  loadMinifiers();
  const absFile = path.join(__dirname, '..', 'public', relPath);
  const ext = path.extname(absFile).toLowerCase();
  if (!MIME[ext] || !fs.existsSync(absFile)) return null;
  const buf = await getMinified(absFile, ext);
  return buf.toString('utf8');
}

module.exports = { minifyStatic, prewarm, minifiedText, minifyJsText };
