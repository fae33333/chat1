...
async function makeThumb(srcAbs, outAbs, w, h, mode = 'cover') {
  const outExt = path.extname(outAbs).toLowerCase();
  const cleanup = () => { try { if (fs.existsSync(outAbs)) fs.unlinkSync(outAbs); } catch (_) { } };

  // 1) sharp أولاً — سريع ومدمج (بلا أدوات أصلية)، يولّد المصغّرة بالصيغة
  //    المطلوبة من امتداد outAbs (webp/jpg/png) بأبعاد دقيقة.
  try {
    const sharp = require('sharp');
    let pipe = sharp(srcAbs, { failOn: 'none' }).rotate(); // محاذاة اتجاه EXIF
    pipe = mode === 'cover'
      ? pipe.resize(w, h, { fit: 'cover', position: 'centre' })
      : pipe.resize(w, h, { fit: 'inside', withoutEnlargement: true });
    if (outExt === '.webp') pipe = pipe.webp({ quality: 82, effort: 4 });
    else if (outExt === '.png') pipe = pipe.png({ compressionLevel: 9 });
  ...
  // 3) jimp (jpg/png فقط) يعمل على أي خادم بدون ImageMagick.
  const jimp = getJimp();
  if (jimp) {
    try {
      const J = jimp.Jimp;
...
async function toWebP(srcPath, maxDim = 512) {
  ...
  try {
    ...
    await sharp(srcPath)
      ...
      .toFile(outPath);
    if (fileSizeOf(outPath) > 0) {
      if (srcPath !== outPath && !srcPath.endsWith('1787401720671_9e07873cf21eb6b88f502a17.png')) {
