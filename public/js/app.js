...
function showRefreshExitBlock() {
  ...
  let icon = '🔄', tag = 'تحديث الصفحة', title = 'هل تريد مغادرة الدردشة؟';
  let msg = 'أنت متواجد في غرفة. يمكنك البقاء، أو الخروج وتحديث الصفحة وإعادة الدخول.';
  if (both) {
...
// اعتراض مفاتيح تحديث الصفحة (F5 / Ctrl+R / Cmd+R) فقط أثناء التواجد في غرفة
...
// اعتراض أي تحديث لا يمرّ عبر لوحة المفاتيح (زر التحديث في المتصفح، إيماءة السحب على الجوال،
// Ctrl+Shift+R…) — نمنع إعادة التحميل مباشرة ونُظهر قالب الاختيار داخل الشات. رفع العلم
// في «الخروج وتحديث الصفحة» يسمح بإعادة التحميل المؤكّدة التي يطلبها المستخدم نفسه.
window.addEventListener('beforeunload', (e) => {
  if (REFRESH_LEAVING) return;
  if (!CUR_ROOM) return;
  e.preventDefault();
  e.returnValue = '';
  showRefreshExitBlock();
