...
function restoreCurrentRoom(socket, attempt = 0) {
  ...
  socket.timeout(2600).emit('join', roomId, ROOM_PWD[roomId] || '', { hidden: !!ROOM_HIDDEN[roomId] }, (error, result) => {
    ...
    if (result && result.ok) {
      ...
      // استدراك الرسائل التي وصلت أثناء الانقطاع — الصفحة تستعيد حالتها «كأنها لم تتجمد»
      if (wasInterrupted) {
        api('/api/rooms/' + roomId + '/messages').then(msgs => {
...
async function chatPingTick() {
  ...
  try {
    const r = await fetch('/api/chat/ping', { headers: { 'X-Chat-Client': '1', 'X-Chat-Token': CHAT_TOKEN }, cache: 'no-store' });
    if (!r.ok) return;
...
function connectSocket() {
  ...
  SOCKET.on('royal_enter', ({ username, avatar, animal, gender }) => { triggerRoyalEntry(username, avatar, animal, gender); });
...
function renderMsg(m) {
  ...
  if (m && m.id) RENDERED_MSG_IDS.add(+m.id);
  const area = $('#msgArea');
  const senderId = +(m.user_id || (m.user && m.user.id) || 0);
  ...
  if (m.type === 'msg') {
    ...
    el.innerHTML = `
      ...
      <div class="mbody">
        ...
        <div class="mline1">
          <span class="mname" data-username="${esc(uname)}" style="color:#000000;font-weight:400">${esc(uname)}${u.verified ? ' <i class="f7-icons vcheck">checkmark_seal_fill</i>' : ''}${u.royal ? ' <i class="f7-icons rcrown">crown_fill</i>' : ''}</span>
...
function triggerRoyalEntry(username, avatar, animal, gender) {
  ...
  // استخدام GIF الحقيقي الذي أرسله المستخدم لكل حيوان
  const gifSrc = a.gif || '';
...
function triggerGiftCelebration(gift) {
  ...
  for (let c = 0; c < 42; c++) {
    ...
    piece.style.setProperty('--color', colors[c % colors.length]);
  ...
  centerPopup.className = 'gift-center-popup giftbox-delayed';
...
function renderUsers() {
  ...
  $('#usersList').innerHTML = list.length ? list.map(u => {
    ...
    <div class="users-row${u.muted ? ' muted-user' : ''}${ignored ? ' ignored-user' : ''}" data-id="${u.id}">
      ...
      <div class="uname" style="color:${userColor(u)};font-weight:${userWeight(u)}">${esc(u.username)}${u.verified ? ' <i class="f7-icons vcheck">checkmark_seal_fill</i>' : ''}${u.royal ? ' <i class="f7-icons rcrown">crown_fill</i>' : ''}${ignored ? `<span class="ignored-user-tag">${APP_LANG === 'en' ? '(Ignored)' : '(متجاهل)'}</span>` : ''}</div>
...
async function openProfile(uid) {
  try {
    ...
    if (isMe) {
      ...
      $('#profTitleTab').innerHTML = `${esc(u.username)} ${u.verified ? '<i class="f7-icons" style="font-size:14px">sparkles</i>' : ''}${u.royal ? '<i class="f7-icons rcrown" style="font-size:14px">crown_fill</i>' : ''}`;
            ...
            <div class="profile-cover-main">
              <div class="profile-cover-hero">
                ...
                <div class="profile-hero-info">
                  <div class="profile-main-name">${esc(u.username)}${u.verified ? '<i class="f7-icons vp-vrf">checkmark_seal_fill</i>' : ''}${u.royal ? ' <i class="f7-icons vp-vrf rcrown">crown_fill</i>' : ''}</div>
...
function beginPrivateCallFlow(callType) {
  ...
  const cost = callType === 'video'
    ? Math.max(0, parseInt(SETTINGS.video_call_cost) || 5)
  ...
  if (goBtn) {
    ...
    goBtn.innerHTML = `<i class="f7-icons">${callType === 'video' ? 'videocam_fill' : 'phone_fill'}</i> ${translateDynamicText('تأكيد وبدء الاتصال')} (${cost} 🪙)`;
    goBtn.onclick = () => {
      closeOv('callConfirmOv');
      executePrivateCall(callType);
    };
...
async function executePrivateCall(callType = 'audio') {
  ...
  try {
    const stream = await navigator.mediaDevices.getUserMedia({
      audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true },
      video: isVideo ? { width: { ideal: 320 }, height: { ideal: 180 }, facingMode: 'user' } : false
    });
    PM_CALL = {
      peerId: +PM_WITH.id,
...
// ===== جودة الفيديو التكيفية (Adaptive Bitrate) — الخامة حسب الإنترنت =====
// مراقبة مستمرة لشبكة WebRTC؛ عند ضعفها تُخفَّض دقة الكاميرا خطوة بخطوة
// (بدل التجمد/التعليق) وتُعاد للرفع عند تحسن الشبكة — المكالمة تبقى سلسة دائماً
// دقات أخف على الهاتف: تبدأ المكالمة بالأخف (180p) حتى لا يتعطل الهاتف،
// وترتفع تدريجياً (حتى 360p كحد أقصى) عندما تكون الشبكة وجهاز التشفير مستقرين
const VIDEO_QUALITY_LEVELS = [
  { w: 320, h: 180, fps: 15, label: 'اقتصادية (شبكة ضعيفة)' },
  { w: 480, h: 270, fps: 20, label: 'منخفضة' },
  { w: 640, h: 360, fps: 20, label: 'عادية' }
];
const VIDEO_QA_START = 0; // البدء بالأخف (180p) حفاظاً على سلاسة المكالمة على كل الهواتف
let VIDEO_QA_TIMER = null;
function setVideoQualityBadge(level) {
  const el = $('#pmVideoQuality');
  if (!el) return;
  // الشارة تظهر فقط عندما تُخفَّض الجودة عن المستوى الافتراضي بسبب الشبكة
  if (level >= VIDEO_QA_START) { el.style.display = 'none'; return; }
  el.textContent = 'الجودة: ' + VIDEO_QUALITY_LEVELS[level].label;
  el.style.display = '';
}
function applyVideoQuality(level) {
  if (!PM_CALL || !PM_CALL.localStream) return;
  const q = VIDEO_QUALITY_LEVELS[level];
  if (!q || PM_CALL.qaLevel === level) return;
  PM_CALL.qaLevel = level;
  PM_CALL.localStream.getVideoTracks().forEach(t => {
    try {
      if (typeof t.applyConstraints === 'function') {
        t.applyConstraints({ width: { ideal: q.w }, height: { ideal: q.h }, frameRate: { ideal: q.fps } }).catch(() => {});
      }
    } catch (e) {}
  });
  setVideoQualityBadge(level);
}
async function videoQualityTick() {
  if (!PM_CALL || PM_CALL.callType !== 'video' || !PM_CALL.pc) { stopVideoQualityMonitor(); return; }
  let stats;
  try { stats = await PM_CALL.pc.getStats(); } catch (e) { return; }
  const st = PM_CALL.qa || (PM_CALL.qa = { level: VIDEO_QA_START, stable: 0, lastDropped: null, lastLost: null });
  let vIn = null, vOut = null, targetBps = 0;
  try {
    stats.forEach(r => {
      if (r.type === 'inbound-rtp' && r.kind === 'video') vIn = r;
      if (r.type === 'outbound-rtp' && r.kind === 'video') vOut = r;
      if (r.type === 'candidate-pair' && r.state === 'succeeded' && r.nominated) {
        const b = +r.targetBitrate || 0;
        if (b > targetBps) targetBps = b;
      }
    });
  } catch (e) { return; }
  // دلتا الإطارات المهدورة والحقوب المفقودة + الاهتزاز
  let droppedDelta = 0, lostDelta = 0, jitter = 0;
  if (vIn) {
    droppedDelta = Math.max(0, (+vIn.framesDropped || 0) - (st.lastDropped == null ? (+vIn.framesDropped || 0) : st.lastDropped));
    lostDelta = Math.max(0, (+vIn.packetsLost || 0) - (st.lastLost == null ? (+vIn.packetsLost || 0) : st.lastLost));
    jitter = +vIn.jitter || 0;
    st.lastDropped = +vIn.framesDropped || 0;
    st.lastLost = +vIn.packetsLost || 0;
  }
  // مستوى الهدف: من تقدير النطاق الترددي إن توفر
  let lvl;
  if (targetBps > 0) {
    lvl = targetBps >= 800000 ? 2 : targetBps >= 350000 ? 1 : 0;
  } else {
    lvl = st.level; // بلا تقدير نطاق: لا نغيّر إلا عند مشاكل واضحة
  }
  // ضغط على الجهاز نفسه: إذا هبطت إطارات التشفير المحلية، الهاتف يثقل → نخفض فوراً
  const outFps = vOut ? (+(vOut.framesPerSecond || vOut.framesSentRate || 0) || 0) : 0;
  const deviceTargetFps = (VIDEO_QUALITY_LEVELS[st.level] || {}).fps || 15;
  const deviceStruggling = !!vOut && outFps > 0 && outFps < deviceTargetFps * 0.55;
  // مشاكل حادة → تخفيض فوري (حتى مع توفر تقدير النطاق)
  if (droppedDelta >= 10 || lostDelta >= 30 || jitter > 0.08 || deviceStruggling) lvl = Math.min(lvl, st.level - 1);
  if (lvl < st.level) {
    applyVideoQuality(lvl);
    st.level = lvl;
    st.stable = 0;
  } else if (lvl > st.level) {
    // الرفع يكون تدريجياً: 3 فترات مستقرة (6 ثوانٍ) بدون مشاكل + الجهاز يتحمل
    if (droppedDelta <= 1 && lostDelta <= 3 && jitter <= 0.03 && (!vOut || outFps === 0 || outFps >= deviceTargetFps * 0.75)) {
      st.stable++;
      if (st.stable >= 3) {
        const next = Math.min(lvl, st.level + 1);
        applyVideoQuality(next);
        st.level = next;
        st.stable = 0;
      }
    } else {
...
function startVideoQualityMonitor() {
  ...
  // أفضلية الحفاظ على عدد الإطارات: عند الازدحام تُخفَّض الدقة قبل السلاسة
  try {
    const sender = PM_CALL.pc.getSenders().find(s => s.track && s.track.kind === 'video');
    if (sender && sender.getParameters) {
      const p = sender.getParameters();
      if (p) { p.degradationPreference = 'maintain-framerate'; sender.setParameters(p).catch(() => {}); }
    }
  } catch (e) {}
  PM_CALL.qa = { level: VIDEO_QA_START, stable: 0, lastDropped: null, lastLost: null };
  PM_CALL.qaLevel = null;
  // تطبيق المستوى الابتدائي الخفيف على الكاميرا مباشرة (حتى قبل أول فحص)
  applyVideoQuality(VIDEO_QA_START);
  setVideoQualityBadge(VIDEO_QA_START);
  VIDEO_QA_TIMER = setInterval(videoQualityTick, 2000);
  videoQualityTick();
}
function stopVideoQualityMonitor() {
  if (VIDEO_QA_TIMER) { clearInterval(VIDEO_QA_TIMER); VIDEO_QA_TIMER = null; }
  if (PM_CALL) PM_CALL.qa = null;
  setVideoQualityBadge(VIDEO_QA_START); // إخفاء الشارة
}
...
function renderCashoutStep2(goldMin, usd) {
  ...
  zone.innerHTML = `
    ...
    <div style="display:flex;gap:8px">
      <button class="gc-btn gc-btn-back" id="gcBackStepBtn" type="button" style="flex:0 0 auto;background:#e5e7ef;color:#475569;padding:13px 16px"><i class="f7-icons">chevron_right</i></button>
...
// =====================================================
const ROYAL_ANIMALS = [
  { key: 'lion', name: 'الأسد الملكي', emoji: '🦁', color: '#f59e0b', desc: 'يدخل كالأسد الهادر — قوة ومهابة', gif: '/img/royal_lion_gif.gif' },
  { key: 'whale', name: 'الحوت الملكي', emoji: '🐋', color: '#38bdf8', desc: 'يبحر في الغرفة بهدوء الملوك — عمق وهدوء', gif: '/img/royal_whale.gif' },
  { key: 'eagle', name: 'العقاب الملكي', emoji: '🦅', color: '#a78bfa', desc: 'يحلّق فوق الجميع — حرية وقوة', gif: '/img/royal_eagle.gif' },
  { key: 'unicorn', name: 'الوحيد قرن', emoji: '🦄', color: '#f472b6', desc: 'يسطع قوس قزح أينما دخل — تميز فريد', gif: '/img/royal_unicorn.gif' }
];
...
function openRoyal() {
  ...
  api('/api/royal-info').then(d => {
    ...
    $('#royalAnimalsGrid').style.display = owned ? 'none' : '';
    ...
    if (owned) note.textContent = 'حسابك يحمل الدخول الملكي 👑 — يظهر توهج حيوانك الملكي عند دخولك أي غرفة، والتاج الذهبي بجوار اسمك أينما ظهرت.';
    else if (d.hasPending) {
      ROYAL_SEL_ANIMAL = d.pendingAnimal || 'lion';
      renderRoyalAnimals();
      $('#royalAnimalsGrid').style.display = 'none';
...
$('#doLogin').onclick = async () => {
  try {
    ...
    // حساب مسجل غير مُفعَّل بريده: يُفعَّل برمز التحقق قبل الدخول
    if (d.needs_verification) { showEmailVerification(d.email, d.tab_token); return; }
...
$('#veResendBtn').onclick = async () => {
  ...
  try {
    const r = await fetch('/api/resend-verify', {
      method: 'POST', credentials: 'same-origin',
...
$('#doRegister').onclick = async () => {
  try {
    ...
    // التسجيل يتطلب تفعيل البريد: ننتقل لشاشة رمز التحقق
    if (d.needs_verification) { showEmailVerification(d.email, d.tab_token, d.verify_sent, d.verify_reason); return; }
    CHAT_TOKEN = d.tab_token || '';
    ME = d.user; MYBADGE = d.badge;
    closeOv('regOv');
    onLoggedIn();
