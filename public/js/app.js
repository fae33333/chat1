...
async function executePrivateCall(callType = 'audio') {
  ...
  try {
    const stream = await navigator.mediaDevices.getUserMedia({
      audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true },
      video: isVideo ? {
        // الحد الأعلى الصريح للمكالمة الخاصة هو 854×480 (480p)، مهما كانت سرعة الشبكة.
        width: { ideal: 320, max: 854 },
...
// ===== جودة الفيديو التكيفية (Adaptive Bitrate) — للمكالمات الخاصة فقط =====
// لا تتجاوز الكاميرا 480p أبداً. تبدأ المكالمة بدقة اقتصادية، ثم ترفعها تدريجياً
// عندما تشير إحصاءات WebRTC إلى أن الشبكة والجهاز قادران على ذلك، وتخفضها فوراً
// عند فقد الحزم أو ازدياد الـ jitter أو ضغط المعالج.
const VIDEO_QUALITY_LEVELS = [
  { w: 320, h: 180, fps: 15, maxBitrate: 180000, label: '180p' },
  { w: 480, h: 270, fps: 20, maxBitrate: 350000, label: '270p' },
  { w: 640, h: 360, fps: 20, maxBitrate: 600000, label: '360p' },
  { w: 854, h: 480, fps: 24, maxBitrate: 1000000, label: '480p' }
];
const VIDEO_QA_START = 0; // بداية اقتصادية حتى لا تُغرق الشبكة قبل ظهور الإحصاءات
const VIDEO_QA_MAX = VIDEO_QUALITY_LEVELS.length - 1; // 480p هو الحد الأعلى الثابت
let VIDEO_QA_TIMER = null;
function setVideoQualityBadge(level) {
  const el = $('#pmVideoQuality');
  if (!el) return;
  const q = VIDEO_QUALITY_LEVELS[Math.max(VIDEO_QA_START, Math.min(VIDEO_QA_MAX, level))];
  el.textContent = 'الجودة: ' + q.label;
  // إبقاء الشارة ظاهرة يوضح للمستخدم أن الدقة تتكيف فعلاً مع الشبكة.
  el.style.display = '';
}
function applyVideoQuality(level) {
  if (!PM_CALL || !PM_CALL.localStream) return;
  level = Math.max(VIDEO_QA_START, Math.min(VIDEO_QA_MAX, +level || 0));
  ...
  PM_CALL.localStream.getVideoTracks().forEach(track => {
    try {
      if (typeof track.applyConstraints === 'function') {
        // max يمنع اختيار دقة أعلى من المستوى الحالي، وخصوصاً أعلى من 480p.
  ...
  // ضبط سقف الإرسال أيضاً؛ لا نعتمد على قيود الكاميرا وحدها في المتصفحات
  // التي تفضّل الاحتفاظ بإطار أكبر عند ازدحام الشبكة.
  try {
...
function bandwidthToVideoLevel(bps) {
  if (!(bps > 0)) return null;
  // نحتاج هامشاً قبل اختيار 480p حتى لا يتذبذب الفيديو عند أول هبوط بسيط.
  if (bps >= 850000) return 3;
  if (bps >= 520000) return 2;
  if (bps >= 300000) return 1;
  return 0;
}
async function videoQualityTick() {
  ...
  let vIn = null, vOut = null, availableBps = 0;
  try {
    stats.forEach(r => {
      if (r.type === 'inbound-rtp' && (r.kind === 'video' || r.mediaType === 'video')) vIn = r;
      if (r.type === 'outbound-rtp' && (r.kind === 'video' || r.mediaType === 'video')) vOut = r;
      if (r.type === 'candidate-pair' && (r.state === 'succeeded' || r.state === 'connected') && (r.nominated || r.selected)) {
        availableBps = Math.max(
          availableBps,
          +r.availableOutgoingBitrate || 0,
          +r.availableIncomingBitrate || 0,
          +r.targetBitrate || 0
        );
      }
    });
  ...
  let level = bandwidthToVideoLevel(availableBps);
  if (level === null) level = st.level;

  const outFps = vOut ? (+(vOut.framesPerSecond || vOut.framesSentRate || 0) || 0) : 0;
  const expectedFps = (VIDEO_QUALITY_LEVELS[st.level] || VIDEO_QUALITY_LEVELS[VIDEO_QA_START]).fps;
  const deviceStruggling = !!vOut && outFps > 0 && outFps < expectedFps * 0.55;
  const hasNetworkProblem = droppedDelta >= 10 || lostDelta >= 30 || jitter > 0.08 || deviceStruggling;

  if (hasNetworkProblem) {
    // لا تسمح مشكلة حادة بإنزال المستوى إلى قيمة سالبة.
    level = Math.max(VIDEO_QA_START, Math.min(level, st.level - 1));
  }

  if (level < st.level) {
    applyVideoQuality(level);
    st.level = level;
    st.stable = 0;
  } else if (level > st.level) {
    // الرفع تدريجي: ثلاث فترات مستقرة (نحو 6 ثوانٍ) لكل خطوة، حتى 480p فقط.
    if (!hasNetworkProblem) {
      st.stable++;
      if (st.stable >= 3) {
        const next = Math.min(VIDEO_QA_MAX, st.level + 1, level);
...
async function loadRoyalAnimals() {
  try {
    ...
    if (d && Array.isArray(d.animals) && d.animals.length) ROYAL_ANIMALS = d.animals;
  } catch (e) { }
}
