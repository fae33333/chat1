...
function renderMsg(m) {
  ...
  if (m.type === 'msg') {
    ...
    // موجة خفيفة متحركة لرسائل الإدارة (رمادي) والسوبر أدمن (زهري فاتح) للتمييز.
    const waveKind = badgeKind === 'superadmin' ? 'msg-super-wave' : (badgeKind === 'admin' ? 'msg-admin-wave' : '');
    el.className = 'msg' + (hiddenAdmin ? ' hidden-admin-msg' : '') + (waveKind ? ' ' + waveKind : '');
    el.innerHTML = `
      <div class="mava">${avatarHtml(u.avatar)}</div>
      <div class="mbody">
        <span class="mwave" aria-hidden="true"></span>
        ${rp ? `
