...
function bcastRenderBar() {
  const bar = $('#liveBar'); const startBtn = $('#liveBarStart'); const muteBtn = $('#liveBarMute'); const speakBtn = $('#liveBarSpeak');
  const hostsBox = $('#liveBarHosts');
  const startIco = startBtn.querySelector('i');
  if (!CUR_ROOM) return;
  ...
  const eligible = !!ME && (isVoiceRoom ? (ME.registered || isAdmRank()) : (isAdmRank() || ['vip', 'premium', 'mmez'].includes(ME.membership)));
  bar.classList.toggle('is-live', !!state);
  muteBtn.hidden = !(state && state.mode === 'audio' && !iAmHost);
  startIco.textContent = isVoiceRoom ? 'mic_fill' : 'videocam_fill';
  ...
  if (hostsBox) {
    ...
    hostsBox.innerHTML = shown.map(h => `<span class="lb-host-chip${pickable ? ' watchable' : ''}" data-hid="${h.id}" title="${esc(h.username)}">${bcastAvatarChip(h.avatar)}</span>`).join('')
  ...
  if (state.mode === 'audio') {
    // غرفة صوتية ببث قائم: أي عضو مؤهل ينضم كمذيع مباشرة دون طلب إذن
    startBtn.hidden = iAmHost || !eligible;
    speakBtn.hidden = true;
  } else {
    // أي عضو مؤهل يمكنه الانضمام كمذيع مشارك ما دام لم يبدأ بثه بعد (سلوك الفيديو كما هو)
    startBtn.hidden = iAmHost || !eligible;
    speakBtn.hidden = true;
  }
  $('#liveLbl').textContent = 'بث مباشر';
  ...
  if (state.mode === 'audio') {
    ...
    bar.onclick = () => {
      if (iAmHost) return openOv('bcastOv');
      if (BCAST && BCAST.roomId === CUR_ROOM.id) return openOv('bcastOv'); // طلب سابق قائم بالفعل
      const hosts = state.hosts || [];
      // مذيع واحد فقط: يمكن فتح تأكيد المشاهدة من الشريط مباشرة. أكثر من مذيع: يجب اختيار صورة مذيع بعينه (chip).
      if (hosts.length === 1) bcastOpenWatchConfirm(hosts[0]);
    };
  }
}
...
async function bcastStart(mode) {
  ...
  SOCKET.emit('bcast:start', CUR_ROOM.id, (res) => {
    ...
    else toast(res.isNewBroadcast ? 'بدأ البث الصوتي — يسمعك جميع من في الغرفة الآن مباشرة' : 'انضممت للبث الصوتي');
...
$('#bcastClose').onclick = () => {
  if (BCAST && BCAST.isHost) return; // المذيع يستخدم زر «إنهاء البث» صراحة، والإغلاق بالـ X يبقي البث شغالاً بالخلفية
  closeOv('bcastOv');
};
...
$('#liveBarSpeak').onclick = () => {
  if (!CUR_ROOM) return;
  if (SPEAK_REQUEST_PENDING) {
    SOCKET.emit('bcast:speak_cancel', CUR_ROOM.id);
    SPEAK_REQUEST_PENDING = false;
    toast('تم إلغاء طلب التحدث', true);
    return bcastRenderBar();
  }
  SOCKET.emit('bcast:speak_request', CUR_ROOM.id, (res) => {
    if (!res || !res.ok) return toast((res && res.text) || 'تعذر إرسال طلب التحدث', false);
    SPEAK_REQUEST_PENDING = true;
    toast('تم إرسال طلب التحدث، بانتظار موافقة المضيف', true);
    bcastRenderBar();
  });
};
