    ...
    bind: async () => {
      ...
      const render = async () => {
        ...
        $('#serviceRequestsList').innerHTML = list.length ? list.map(r => {
          ...
          const title = isGold ? `طلب شراء رصيد (${r.suggested_gold || 0} ذهب)` : (isVerify ? 'طلب توثيق الحساب' : (isRoyal ? '👑 طلب دخول ملكي' : `طلب ترقية إلى ${String(r.plan || '').toUpperCase()}`));
          const icon = isGold ? 'money_dollar_circle_fill' : (isVerify ? 'checkmark_seal_fill' : (isRoyal ? 'crown_fill' : 'crown_fill'));
          const iconColor = isGold ? '#f59e0b' : (isVerify ? '#2563eb' : (isRoyal ? '#b45309' : '#7c3aed'));
          const iconBg = isGold ? '#fef3c7' : (isVerify ? '#dbeafe' : (isRoyal ? '#fef3c7' : '#ede9fe'));
          const borderColor = isGold ? '#fde68a' : (isVerify ? '#bfdbfe' : (isRoyal ? '#fcd34d' : '#ddd6fe'));
          let details;
          if (isGold) details = `المستخدم: ${esc(r.username)} • الكمية المطلوبة: ${r.suggested_gold || 0} ذهب`;
            ...
            details = `المستخدم: ${esc(r.username)} • الحيوان الملكي: ${ra[0]} ${ra[1]} • توهج الدخول + شارة التاج الدائمة`;
