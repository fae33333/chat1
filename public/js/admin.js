...
const ADMIN_I18N_EN = {
  ...
  "معلومات ترخيص النظام": "System License Information",
  "رقم الترخيص": "License Key",
  "اسم النطاق / IP": "Domain / IP",
  "البريد الإلكتروني": "Email",
  "التعريفات": "Definitions / Limits",
...
const ADMIN_I18N_ES = {
  ...
  "معلومات الترخيص": "Información de Licencia",
  ...
  "معلومات ترخيص النظام": "Información de Licencia del Sistema",
  "رقم الترخيص": "Clave de Licencia",
  "اسم النطاق / IP": "Dominio / IP",
  "البريد الإلكتروني": "Correo Electrónico",
  "التعريفات": "Definiciones / Límites",
  "استخدام غير محدود": "Uso Ilimitado",
  "الإصدار": "Versión",
  "مرخص ✅": "Licenciado ✅",
  "حفظ": "Guardar",
  "تعديل": "Editar",
...
const ADMIN_I18N_TR = {
  ...
  "معلومات الترخيص": "Lisans Bilgileri",
  ...
  "معلومات ترخيص النظام": "Sistem Lisans Bilgileri",
  "رقم الترخيص": "Lisans Numarası",
  "اسم النطاق / IP": "Alan Adı / IP",
  "البريد الإلكتروني": "E-posta",
  "التعريفات": "Tanımlar / Limitler",
  "استخدام غير محدود": "Sınırsız Kullanım",
  "الإصدار": "Sürüm",
  "مرخص ✅": "Lisanslı ✅",
  "حفظ": "Kaydet",
  "تعديل": "Düzenle",
...
const MENU = [
  ...
  { icon: 'chat_bubble_2_fill', color: '#ec4899', label: 'مراقبة الخاص والمكالمات', masterOnly: true, subs: [
    ...
    { id: 'callsRecordings', icon: 'phone_waveform_fill', label: 'تسجيلات المكالمات', masterOnly: true }]},
...
const PAGES = {
  ...
  memberships: {
    build: () => `
      ...
      <div class="section" style="border:0;box-shadow:none;padding:0">
        ...
        ${inpRow('phone_fill', '#10b981', 'تكلفة المكالمة الصوتية المفتوحة (بالذهب)', 'call_cost', 'number', 'ذهب')}
        <div class="btn-row">
    ...
    bind: () => {
      $('#saveMem').onclick = async () => {
        await saveKeys(['vip_cost', 'premium_cost', 'plus_cost', 'register_gold', 'call_cost']);
        toast('تم حفظ إعدادات رصيد العضويات والتسجيل بنجاح');
      };
      $('#resetMem').onclick = async () => {
        SETTINGS = { ...SETTINGS, vip_cost: '30', premium_cost: '20', plus_cost: '10', register_gold: '10', call_cost: '2' };
        await saveKeys(['vip_cost', 'premium_cost', 'plus_cost', 'register_gold', 'call_cost']);
    ...
    bind: async () => {
      ...
      const render = async () => {
        ...
        $('#serviceRequestsList').innerHTML = list.length ? list.map(r => {
          ...
          const title = isGold ? `طلب شراء رصيد (${r.suggested_gold || 0} ذهب)` : (isVerify ? 'طلب توثيق الحساب' : `طلب ترقية إلى ${String(r.plan || '').toUpperCase()}`);
          const icon = isGold ? 'money_dollar_circle_fill' : (isVerify ? 'checkmark_seal_fill' : 'crown_fill');
          const iconColor = isGold ? '#f59e0b' : (isVerify ? '#2563eb' : '#7c3aed');
          const iconBg = isGold ? '#fef3c7' : (isVerify ? '#dbeafe' : '#ede9fe');
          const borderColor = isGold ? '#fde68a' : (isVerify ? '#bfdbfe' : '#ddd6fe');
          const details = isGold
            ? `المستخدم: ${esc(r.username)} • الكمية المطلوبة: ${r.suggested_gold || 0} ذهب`
            : (isVerify ? `المستخدم: ${esc(r.username)}` : `صاحب الطلب: ${esc(r.username)} • الحساب المستهدف: ${esc(r.target_name)} • المدة: ${r.months} شهر`);
          return `<div class="section" style="margin-bottom:12px;border-color:${borderColor}">
            ...
            ${currentStatus === 'pending' ? `<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:14px;padding-top:12px;border-top:1px solid #edf0f5">
              ...
              <input class="inp num request-gold" data-id="${r.id}" type="number" min="1" max="100000" value="${r.suggested_gold || (isGold ? 10 : 0)}" style="width:120px">
  ...
  seoArchive: {
    build: () => `
      ...
      <div class="section">
        ...
        <div id="seoFormContainer" style="display:none;background:#f8fafc;border:1.5px dashed #cbd5e1;border-radius:14px;padding:16px;margin-bottom:18px">
          ...
          <div class="fgroup">
            <label>أيقونة الفافيكون المخصصة لهذا المسار (Favicon):</label>
            <div style="display:flex;align-items:center;gap:8px">
              <input class="inp" id="seoPageFavicon" placeholder="مثال: /uploads/favicon.png" style="flex:1">
    ...
    bind: async () => {
      ...
      $('#saveSeoPageBtn').onclick = async () => {
        ...
        try {
          await api('/api/admin/seo-pages', 'POST', {
            id: id ? +id : undefined,
            slug,
            title,
  ...
  callsRecordings: {
    build: () => `
      <div class="page-title"><i class="f7-icons mi" style="color:#10b981">phone_waveform_fill</i> تسجيلات المكالمات الخاصة</div>
      <div class="section-title"><i class="f7-icons mi" style="color:#94a3b8">list_bullet</i> قائمة المكالمات الصوتية المسجلة بالأسماء</div>
      <div class="fgroup" style="margin-bottom:14px">
    ...
    bind: async () => {
      let recs = [];
      try { recs = await api('/api/admin/call-recordings'); } catch (e) { recs = []; }
      ...
      const renderCalls = (filter = '') => {
        ...
        if (!filtered.length) {
          $('#callsList').innerHTML = '<div class="empty">⏳ لا توجد تسجيلات مكالمات بعد</div>';
          return;
        }
            ...
            <div class="pm-call-rec-card">
              <div class="pm-call-rec-head">
                ...
                <div class="pm-call-rec-actions">
                  <a href="${esc(r.audio_path)}" download="${esc(r.filename)}" class="btn btn-purple btn-sm" style="text-decoration:none;display:inline-flex;align-items:center;gap:4px">
              ...
              <div class="pm-call-rec-player">
                <audio src="${esc(r.audio_path)}" controls preload="metadata"></audio>
              </div>
            </div>`;
        }).join('');
      };
...
const ADMIN_ALLOWED_PAGES = new Set(['roomAdd', 'userAdd', 'kicks', 'bans', 'broadcast', 'words', 'verified', 'license']);
